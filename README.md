...
### Поколения протокола AmneziaWG: `main.go` (корень) и `gen3/`
Репозиторий содержит **два независимых сборочных модуля** для двух поколений контейнера AmneziaWG:
- **Корень** (`main.go`, этот README) — под классический контейнер `amnezia-awg` (интерфейс `wg0`, `wg0.conf`), актуален для уже действующих серверов на этом поколении протокола.
- **`gen3/`** — отдельный Go-модуль (свой `go.mod`) под актуальное поколение протокола 3.1 (`amneziavpn/amneziawg-go`, контейнер обычно называется `amnezia-awg2`, интерфейс `awg0`, `awg0.conf`, конфиг клиента дополнительно копирует поля обфускации `S3`/`S4`). Собирается и разворачивается так же (`cd gen3 && go build ...`), но НЕ совместим по формату конфига/интерфейса с корневой сборкой — не путать при установке.

Оба модуля используют одни и те же `static/*` (UI идентичен) и одинаковую схему версии (`AppVersion` в своём `main.go`).
