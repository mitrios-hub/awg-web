...
Панель рассчитана на **актуальное поколение протокола AmneziaWG** (образ `amneziavpn/amneziawg-go`, контейнер по умолчанию `amnezia-awg2`, интерфейс `awg0`, конфиг `/opt/amnezia/awg/awg0.conf`). Отдельная сборка под старое поколение (`amnezia-awg`/`wg0`) больше не поддерживается и из репозитория убрана; имя контейнера, интерфейс и пути всё так же задаются в `config.json`, если у вас они другие.
